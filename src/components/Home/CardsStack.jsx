...
// PIN_TOP: where each card sticks (px from viewport top).
// Each card sits 34px lower than the previous so you see the stack building.
const PIN_TOPS = cards.map((_, i) => 100 + i * 34);
// Final scale each card shrinks to when the next one slides on top of it.
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      cardEls.forEach((card, index) => {
        ...
        // Pin this card when its top hits pinTop px from viewport top.
        // Keep it pinned until the whole section ends.
        ...
        // Scale THIS card down as the NEXT card scrolls in.
    ...
    <section ref={sectionRef} className="cs-section">
      <div className="cs-wrapper">
        {cards.map((card, index) => (
          ...
          >
            {/* Big title above the card line */}
            <div className="cs-card-header">
              <h2 className="cs-card-title" style={{ color: card.titleColor }}>
                {card.title}
              </h2>
              <svg
            ...
            {/* Divider */}
            <div className="cs-divider" style={{ backgroundColor: card.borderColor }} />

            {/* Body: services list + gradient visual */}
            <div className="cs-body">
              <ul className="cs-services">
                {card.services.map((s, i) => (
                  <li key={i} style={{ color: card.titleColor }}>
                    {s}
                  </li>
                ))}
              </ul>
